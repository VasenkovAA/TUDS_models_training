...
class UNetConv(nn.Module):
    def __init__(
        ...
        self._model = nn.Sequential(
            ...
            nn.LeakyReLU(0.2),
            nn.Conv1d(out_channels, out_channels, kernel_size=kernel_size, padding=padding, stride=stride),
            nn.BatchNorm1d(out_channels),
            nn.LeakyReLU(0.2),
        )
...
class UNet1d(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        # UNet++ connections with additional processing layers
        x = self._up1(x6, x)
        x = self._up1_1(x) + x
        x = self._up2(x5, x)
        x = self._up2_1(x) + x
